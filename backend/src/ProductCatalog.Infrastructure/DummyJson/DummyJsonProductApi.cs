...
public class DummyJsonProductApi : IDummyJsonProductApi
{
	private readonly HttpClient _httpClient;
	private readonly ILogger<DummyJsonProductApi> _logger;

	public DummyJsonProductApi(HttpClient httpClient, ILogger<DummyJsonProductApi> logger)
	{
	...
	public async Task<ServiceResponse<PagedResult<ProductListItemDto>>> GetProductsAsync(ProductQuery query, CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			if (hasCategory)
			{
				products = products.Where(p => string.Equals(p.Category, query.Category, StringComparison.OrdinalIgnoreCase));
			}
			...
			if (needsManualPaging)
			{
				...
				return ServiceResponse<PagedResult<ProductListItemDto>>.Ok(new PagedResult<ProductListItemDto>
				{
					Items = pageItems,
					Page = query.Page,
